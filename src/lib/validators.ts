...
// === Chatbot ===
// Public lead-gate form. The visitor fills this in BEFORE chatting.
// We re-use the same field shape as `leadWriteSchema` so the resulting
// lead row looks identical to leads captured by any other channel
// (contact form, manual entry, RFQ, etc.) — admin views shouldn't
// have to special-case chatbot leads.
export const chatbotLeadGateSchema = z.object({
  company_name: z.string().min(1).max(255),
  contact_person: z.string().min(1).max(255),
  email: z.string().email().max(255),
  phone: z.string().max(50).optional().nullable(),
  ...
  // Not a session/auth token — anyone with the lead_id can read the
  // transcript, by design.
  visitor_token: z.string().min(8).max(128),
});

export type ChatbotLeadGatePayload = z.infer<typeof chatbotLeadGateSchema>;
